...
// True only when real live/test keys are configured.
function isConfigured() {
  return Boolean(keyId() && keySecret());
}

// The dev bypass lets /api/orders/:id/verify (see routes/orders.js) accept a
// payment without checking a Razorpay signature, purely so a developer with
// no keys configured can click through checkout locally. It must never be
// reachable just because keys happen to be blank -- that is exactly the
// shape of a misconfigured production deploy quietly accepting free orders
// from anyone who calls the API directly. So it requires BOTH:
//   1. An explicit, named opt-in: RAZORPAY_DEV_BYPASS must be the literal
//      string "true". Absent, empty, or any other value keeps it off.
//   2. NODE_ENV is not "production" -- checked here, unconditionally, so a
//      stale RAZORPAY_DEV_BYPASS=true left over in a shared or copied .env
//      can never reach a production process.
function devBypassEnabled() {
  if (process.env.NODE_ENV === 'production') return false;
  return process.env.RAZORPAY_DEV_BYPASS === 'true';
}
